...
if (admin.apps.length === 0 && FIREBASE_SERVICE_ACCOUNT_JSON_STRING) {
  try {
    ...
    console.log("Firebase Admin SDK for create-mercadopago-preference route initialized.");
  } catch (e: any) {
    console.error('Firebase Admin SDK Initialization Error in create-mercadopago-preference route:', e.stack);
  }
}
...
async function verifyFirebaseToken(request: NextRequest): Promise<admin.auth.DecodedIdToken | null> {
  ...
  try {
    ...
    console.error('Error verifying Firebase ID token in create-mercadopago-preference route:', error);
    return null;
  }
}
