// /pages/api/create-mercadopago-preference.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!MERCADOPAGO_ACCESS_TOKEN) {
        return res.status(503).json({ error: 'La pasarela de pago no está configurada correctamente.' });
    }
    ...
    try {
        ...
        if (!speiDetails || !speiDetails.bank_transfer?.clabe) {
            console.error('❌ SPEI details (CLABE) not found in Mercado Pago response:', result.id);
            return res.status(500).json({ error: 'No se pudieron obtener los detalles para la transferencia SPEI.' });
        }
        ...
        return res.status(500).json({ error: `No se pudo crear la orden de pago: ${errorMessage}` });
    }
}
