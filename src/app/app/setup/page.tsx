...
const SetupPage = () => {
  ...
  const getValidationMessage = (): string => {
    ...
    if (isReconfiguring) { // Reconfiguring Flow: Step 1 (Details), Step 2 (DB if needed OR Plan), Step 3 (Plan if DB was Step 2)
      switch (currentValidationStep) {
        ...
        case 2: // DB Config (if needed) OR Plan Selection
          if (dbNeeded) { // Validating DB Config
            ...
            if (databaseOption.type === "excel") {
                ...
                if (!databaseOption.file && !(existingDb && existingDb.source === 'excel' && existingDb.details === databaseOption.name)) { // if no new file and not reconfig an existing excel
                   if(!databaseOption.accessUrl?.startsWith('https://docs.google.com/spreadsheets/')) { // if not converted to gsheet yet
                     return "Por favor, sube un archivo de Excel o procesa el existente para generar un Google Sheet.";
                   }
                }
                if (databaseOption.file && !databaseOption.name?.trim()) return "El nombre del archivo Excel es requerido.";
            }
    ...
    } else { // Standard Setup Flow: Step 1 (Details), Step 2 (DB if needed OR Auth), Step 3 (Auth if DB OR Plan), Step 4 (Plan if DB and Auth)
      switch (currentValidationStep) {
        case 1: // Assistant Details
          if (!assistantName.trim()) return "Por favor, ingresa un nombre para el asistente.";
          if (selectedPurposes.size === 0) return "Por favor, selecciona al menos un propósito para tu asistente.";
        ...
        case 2: // DB Config (if needed) OR Auth
           if (dbNeeded) { // Validating DB Config
            ...
            if (databaseOption.type === "excel") {
                if (!databaseOption.file && !databaseOption.accessUrl?.startsWith('https://docs.google.com/spreadsheets/')) return "Por favor, sube un archivo de Excel o procesa uno para generar un Google Sheet.";
                if (databaseOption.file && !databaseOption.name?.trim()) return "El nombre del archivo Excel es requerido.";
            }
            if (databaseOption.type === "smart_db" && !databaseOption.name?.trim()) return "Por favor, proporciona un nombre para tu Base de Datos Inteligente.";
        ...
        case 3: // Auth (if DB was configured) OR Plan
          if (dbNeeded) { // Validating Auth (as Step 3)
            if (!authMethod) return "Por favor, autentica tu cuenta o elige continuar sin cuenta.";
  ...
  const isStepValid = (): boolean => {
    ...
    if (isReconfiguring) { // Reconfiguring Flow
      switch (currentValidationStep) {
        case 1: return assistantName.trim() !== '' && selectedPurposes.size > 0;
        case 2: 
          if (dbNeeded) { 
            if (!databaseOption.type) return false;
            if (databaseOption.type === "google_sheets") return !!databaseOption.name?.trim() && !!databaseOption.accessUrl?.trim() && databaseOption.accessUrl.startsWith('https://docs.google.com/spreadsheets/');
            if (databaseOption.type === "excel") {
                // Valid if an Excel file is present and has a name, OR if an accessUrl (converted GSheet) is present.
                return (!!databaseOption.file && !!databaseOption.name?.trim()) || (!!databaseOption.accessUrl?.startsWith('https://docs.google.com/spreadsheets/'));
            }
      ...
      switch (currentValidationStep) {
        ...
        case 2: 
          if (dbNeeded) { 
            if (!databaseOption.type) return false;
            if (databaseOption.type === "google_sheets") return !!databaseOption.name?.trim() && !!databaseOption.accessUrl?.trim() && databaseOption.accessUrl.startsWith('https://docs.google.com/spreadsheets/');
            if (databaseOption.type === "excel") return (!!databaseOption.file && !!databaseOption.name?.trim()) || (!!databaseOption.accessUrl?.startsWith('https://docs.google.com/spreadsheets/'));
            if (databaseOption.type === "smart_db") return !!databaseOption.name?.trim();
            return true;
          } else { 
            return !!authMethod;
  ...
  const handlePrevious = () => {
    ...
    else if ((currentStep === 3 && !isReconfiguring) || (currentStep === 2 && isReconfiguring)) { 
      if (!needsDatabaseConfiguration()) {
        dispatch({ type: 'SET_WIZARD_STEP', payload: 1 });
      } else {
  ...
  const handleCompleteSetup = async () => {
    ...
    if (needsDatabaseConfiguration() && databaseOption.type) {
      ...
      newDbEntries.push({
        ...
        source: databaseOption.type, // This could be 'excel' or 'google_sheets' if converted
        details: databaseOption.type === 'excel' && databaseOption.file && !databaseOption.accessUrl // Store original Excel filename if not converted
            ? databaseOption.file.name 
            : databaseOption.name, // Otherwise, use the (potentially GSheet) name
        accessUrl: databaseOption.accessUrl || undefined,
      });
      newAssistantDbIdToLink = dbId;
    } else if (!needsDatabaseConfiguration()) {
        dispatch({ type: 'SET_DATABASE_OPTION', payload: { type: null, name: '', file: null, accessUrl: '' } });
    }
  ...
  const renderStepContent = () => {
    ...
    if (isReconfiguring) {
      if (!dbNeeded) { // Reconfig: Details (1), Plan (2)
        if (currentStep === 2) stepToRender = 3; // Visually step 2 is Plan (which is original step 4)
      }
      // If DB needed: Details (1), DB (2), Plan (3)
      // currentStep maps to visual step
      ...
      if (!dbNeeded) { // Standard: Details (1), Auth (2), Plan (3)
      ...
      // If DB needed: Details (1), DB (2), Auth (3), Plan (4)
      // currentStep maps to visual step
